...
public class DanySlime : Enemy
{
    public GameObject JoseObject;
    public Vector3 rangeOffset;
    private Vector2 originalPosition;

    public GameObject blueSlimeParticles;
    ...
    private void dies()
    {
        ...
        if (randomValue >= 1 && randomValue <= 3) //30% change of Spawning slimeDrop after Death
        {

            Instantiate(slimeDrop,transform.position,Quaternion.identity);
        } else if (randomValue >= 4 && randomValue <= 8) //50% change of Spawning a coin after Death
        {
    ...
    void gotHit()
    {
        if (health > 0 )
        {
            Instantiate(blueSlimeParticles, transform.position, Quaternion.identity); //Instantiates on hit particles
        }
        else
        {
            Instantiate(deathParticles, transform.position, Quaternion.identity); //Instantiates on death particles
        }
    }
